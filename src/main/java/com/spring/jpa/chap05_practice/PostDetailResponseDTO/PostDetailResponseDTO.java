package com.spring.jpa.chap05_practice.PostDetailResponseDTO;
...
public class PostDetailResponseDTO {
    ...
    public PostDetailResponseDTO(Post post) {
        ...
        //HashTag 같은 경우에는 가공해서 집어넣어야 합니다. (리스트의 타입이 서로다름)
//        List<String> list = new ArrayList<>();
//        for(HashTag ht : post.getHashTags()){
//            list.add(ht.getTagName());
//        }
//        this.hasTags=list;
        this.hasTags=post.getHashTags() //List<HashTag>
                .stream()//Stream 객체를 받아옴 (컬렉션 데이터를 함수선언 형식으로 처리할수있게 해주는 객체)
                .map(HashTag::getTagName)//스트림 내의 요소들에 대해 함수가 적용된 결과를 새로운 요소로 맵핑.
